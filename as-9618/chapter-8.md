...
This chapter should be taught as a **scenario-answering chapter**, not as a purely theoretical chapter. Students must be able to read a mini-case study, identify tables, find keys, explain relationships and write short SQL scripts.

## Suggested lesson order

1. Start with a messy file-based system.
2. Ask students to identify repeated data.
3. Convert the repeated data into separate tables.
4. Add primary keys and foreign keys.
5. Draw a simple E-R diagram.
6. Introduce normalisation only after students understand redundancy.
7. Practise SQL DDL before DML.
8. Use 2024–2025 style mini database scenarios.

## What to emphasise for marks

+ For key definitions, students must use exact wording: **uniquely identifies**, **references primary key**, **foreign key values match primary key values**.
+ For SQL, Cambridge usually rewards structure. A nearly correct SQL answer can still gain partial marks if field names, conditions and functions are correct.
+ Students should not use brand-specific SQL.
+ IDs with leading zeros should be treated as text, not integer.
+ Linking tables are the key to many-to-many questions.

## Common student weakness

Students often understand SQL in class but lose marks because:

+ they miss semicolons or brackets
+ they forget `FROM`
+ they use `SUM` when they need `COUNT`
+ they use `COUNT` when they need `SUM`
+ they forget `WHERE`
+ they make IDs numeric when they contain letters or leading zeros
+ they describe normalisation too vaguely

## Teacher quick activity

Give students this table:

```text
ORDER(OrderID, CustomerName, CustomerPhone, ProductID, ProductName, Quantity)
```

Ask:

1. What data is repeated?
2. What tables should be created?
3. What are the primary keys?
4. What foreign keys are needed?
5. Write a `CREATE TABLE` statement for `ORDER_ITEM`.

---

# 16. One-Page Exam Sheet
...
| Attribute | field / column |
| Entity | object about which data is stored |
| DBMS | software to define, create, maintain and control database |
| Data dictionary | stores metadata / data about data |
| Normalisation | process of organising tables to reduce redundancy |
...
| --- | --- |
| 1NF | no repeating groups; atomic values |
| 2NF | 1NF + every non-key attribute depends on the whole key |
...
CREATE TABLE TableName (
    ...
    Field2 INTEGER,
    PRIMARY KEY (Field1),
    FOREIGN KEY (Field2) REFERENCES OtherTable(Field2)
);
```
...
+ `VARCHAR` for IDs with letters or leading zeros.
+ `REAL` for money / price.
+ `BOOLEAN` for true/false.
+ Always include `WHERE` in `UPDATE` and `DELETE`.
+ Linking table for many-to-many.
+ Do not define foreign key without saying what table it references.
...
- [ ] explain one limitation of file-based approach
- [ ] explain how relational database solves that limitation
- [ ] define primary key, candidate key, foreign key
- [ ] define referential integrity and tuple
- [ ] identify one-to-many and many-to-many relationships
- [ ] explain why a linking table is needed
- [ ] explain the purpose of normalisation
- [ ] recognise 1NF, 2NF and 3NF problems
- [ ] describe DBMS features
- [ ] describe data dictionary with examples
- [ ] distinguish DDL and DML
- [ ] write `CREATE TABLE`
- [ ] write `SELECT ... FROM ... WHERE`
- [ ] use `SUM`, `COUNT`, `AVG`
- [ ] use date conditions
- [ ] use `GROUP BY`
- [ ] write a simple join
