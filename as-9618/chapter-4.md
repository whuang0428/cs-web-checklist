...
1. Start with a visual CPU diagram: CPU, memory, buses, registers.
2. Teach register roles using a “who stores what?” table.
3. Teach FDE cycle as register transfers, not as vague fetch/decode/execute words.
4. Use a class activity where students act as PC, MAR, MDR, CIR, CU and memory.
5. Teach addressing modes using the same memory table.
6. Do assembly trace questions every lesson for 5 minutes.
7. Teach bit manipulation with physical switches or 8-bit sensor examples.
8. End with interrupt sequencing because students need precise order.

## 21.2 High-value classroom activities

| Activity | Purpose |
| --- | --- |
| Register role matching | stops PC/MAR/MDR/CIR confusion |
| FDE card sort | trains correct sequence |
| Bus colour coding | separates address/data/control bus |
| Assembly trace mini-drills | prepares for 4–6 mark trace questions |
| Addressing mode sorting | prevents immediate/direct/indirect mistakes |
| Bit mask lab | makes AND/OR/XOR meaningful |
| Examiner-report correction | improves vague answers |

## 21.3 Marking guidance for students

Train students to avoid name-repeating answers:

| Weak | Strong |
| --- | --- |
| The status register stores status | SR stores status flags as individual bits, such as zero/carry/overflow |
| The control unit controls things | CU decodes instructions and sends control signals to coordinate CPU operations |
| General purpose registers are general | General purpose registers can temporarily store different data values during program execution |
| Dual core is faster | Dual core can process tasks in parallel if the software supports it |

## 21.4 What to assess weekly

+ Can students write FDE sequence from memory?
+ Can students trace ACC and IX correctly?
+ Can students identify addressing modes from syntax?
+ Can students explain SR using flags and bits?
+ Can students use AND/OR/XOR masks correctly?
...
+ **CU**: decodes instructions, sends control signals  
+ **System clock**: synchronises operations using timing signals  
+ **IAS / main memory**: stores current programs and data  
...
| --- | --- |
| PC | address of next instruction |
| MAR | address of memory location being accessed |
| MDR | data/instruction transferred to/from memory |
| CIR | current instruction |
| ACC | calculation results / intermediate values |
| IX | offset for indexed addressing |
| SR | status flags as individual bits |

## Buses

+ **Address bus**: carries memory address, usually CPU → memory  
+ **Data bus**: carries data/instructions, bi-directional  
...
5. MDR → CIR  
6. PC incremented  
7. CU decodes instruction  
8. Instruction executed  

## Interrupts

1. Interrupt flag raised  
2. CPU completes current FDE cycle  
3. Priority checked  
4. Register contents saved on stack  
5. ISR called  
6. Registers restored  
...
| Mode | Meaning |
| --- | --- |
| Immediate | operand is actual value, e.g. `#5` |
| Direct | operand is address |
| Indirect | operand address stores another address |
| Indexed | effective address = operand + IX |
| Relative | address relative to PC |

## Bit manipulation

+ **AND mask**: test / isolate bit  
+ **OR mask**: set bit  
+ **XOR mask**: toggle bit  
+ **Logical shift**: zeros shifted in  
+ **Arithmetic right shift**: sign bit preserved  
+ **Cyclic shift**: shifted-out bit re-enters at other end  

## Last-minute warnings

+ PC stores **address**, not instruction.  
+ MDR stores **data/instruction**, not address.  
+ SR answer must mention **flags / individual bits**.  
+ `#` usually means **immediate value**.  
+ Dual-core is not automatically twice as fast.  
+ Always trace assembly line by line.  
