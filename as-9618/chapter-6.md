...
1. Start with three core definitions: **security / privacy / integrity**.  
2. Teach threats using mini-scenarios: fake email, fake website, spyware, hacker.  
3. Teach protection methods in matched pairs:
   - phishing → user education / anti-phishing awareness / avoid suspicious links
   - malware → anti-virus / updates
   - unauthorised access → authentication / access rights / firewall
   - intercepted data → encryption
4. Teach validation using table examples from account forms.
5. Teach verification using two separate contexts:
   - data entry: visual check, double entry
   - data transfer: parity, checksum
6. Finish with Paper 1-style bank/website scenarios.

## 16.2 What to reduce from older teaching

| Reduce | Why |
| --- | --- |
| memorising many exotic malware types | syllabus mainly names virus and spyware |
| advanced encryption mathematics | AS Chapter 6 only expects general principle |
| long privacy law essays | not usually rewarded in Chapter 6 Paper 1 answers |
| generic "keep data safe" statements | mark schemes reward specific methods and effects |
...
- not just **"firewall protects data"**, but **"filters traffic and blocks unauthorised access using rules"**
- not just **"encryption makes data safe"**, but **"scrambles data so intercepted data cannot be understood without a key"**
- not just **"validation checks data"**, but **"checks data is reasonable and meets rules, not necessarily correct"**
- not just **"checksum checks data"**, but **"sender calculates, receiver recalculates, values compared"**
- not just **"biometric uses face"**, but **"compares captured biological feature with stored template"**

---

# 17. Final One-Page Exam Sheet

## Core terms

- **Security** = protection from unauthorised access / damage / misuse  
- **Privacy** = personal/sensitive data accessed only by authorised users  
- **Integrity** = data is accurate, complete, consistent, not altered  

## Threats

- **Virus** = malware that replicates / attaches to files  
- **Spyware** = secretly monitors and collects user data  
- **Hacker** = unauthorised access attempt  
- **Phishing** = fake message tricks user into giving confidential data  
...
- **Biometrics** compare biological feature with stored template  
- **Firewall** monitors/filters traffic and blocks unauthorised access  
- **Anti-virus** detects/quarantines/removes malware  
- **Anti-spyware** detects/removes spyware  
- **Encryption** scrambles data; needs key to decrypt  
- **Access rights** restrict files/functions users can access  

## Validation

- **Range check** = lower and upper limit  
- **Limit check** = one boundary  
- **Length check** = number of characters  
- **Format check** = pattern  
- **Presence check** = not blank  
- **Existence check** = exists in file/list/database  
- **Check digit** = calculated digit appended and recalculated  

## Verification

- **Visual check** = compare with original source  
- **Double entry** = enter twice and compare  
- **Parity check** = add parity bit to make 1s odd/even  
- **Block parity** = horizontal + vertical parity; sends parity byte  
- **Checksum** = sender calculates value from data; receiver recalculates and compares  

## Exam answer patterns

### Firewall

> Monitors incoming/outgoing traffic; applies filtering rules; blocks unauthorised/suspicious traffic.

### Encryption

> Encodes/scrambles data; if intercepted it cannot be understood without the key.

### Checksum

> Sender calculates checksum and sends it with data; receiver recalculates; values are compared; mismatch means error.

### Validation vs verification

> Validation checks data is reasonable; verification checks data was copied/transferred accurately.
