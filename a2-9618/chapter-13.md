...
1. Start with **record / enum / set pseudocode templates**. Students can gain easy marks if syntax is clean.  
2. Then teach **file organisation** with real scenarios: transaction log, sorted account file, random stock lookup.  
3. Teach **hashing and collisions** as a direct-access method, not as a general programming hash table lesson.  
4. Spend the largest calculation time on **floating-point conversion**, especially negative values.  
5. End with **precision vs range** because it is a common short explanation question.

## 13.2 High-value classroom activities

| Activity | Purpose |
| --- | --- |
| 5-minute UDT syntax drill | improve `TYPE`, `DECLARE`, `ENDTYPE`, `SET OF`, `DEFINE` accuracy |
| Data type choice table | train students to choose `STRING`, `INTEGER`, `DATE`, enum correctly |
| File scenario sorting | match serial/sequential/random to practical situations |
| Hash collision role-play | show why two keys can produce same location |
...
+ File organisation questions often reward exact contrast words: **chronological**, **key field**, **physical start**, **EOF**, **hash value**, **collision**.
+ For sets, Cambridge expects **unordered elements**, **same data type** and **set theory operations**.
+ For enumerated type, train students to say **fixed range/list of possible values**.

## 13.4 Marking guidance for students

Students should be trained to write:

+ not just **"record stores data"**, but **"record is a user-defined composite data type containing fields that may be different data types"**
+ not just **"enum is a list"**, but **"fixed list of possible values"**
+ not just **"set is a group"**, but **"unordered elements of the same data type with set theory operations"**
+ not just **"hashing finds a place"**, but **"record key is processed to calculate a storage location"**
+ not just **"collision means same place"**, but **"two different keys produce the same hash value / storage location"**
+ not just **"more exponent is better"**, but **"more exponent bits gives greater range; more mantissa bits gives greater precision"**

---

# 14. Final One-Page Exam Sheet

## User-defined data types
+ **Non-composite**: defined without referencing another data type.  
+ Examples: **enumerated**, **pointer**.  
+ **Composite**: contains / refers to other data type(s).  
...
TYPE Vehicle = (M100, M230, T101, T102)
```
Use when there is a **fixed range of possible values**.

## Pointer type
```text
TYPE TPointer = ^STRING
```
Stores a **memory address** and indicates the **type of data** stored there.
...
ENDTYPE
```
Record = fields grouped under one identifier; fields may have different data types.
...
DEFINE Operators ('+', '–', '*', '/', '^') : SymbolSet
```
Set = unordered elements, same data type, union/intersection/difference.

## File organisation
| Type | Key phrase |
| --- | --- |
| Serial | records stored in order added / chronological |
| Sequential | records stored in order of key field |
| Random | location based on record key / hashing |

## File access
+ **Sequential access**: records searched one after another from physical start until found / EOF.  
+ **Direct access**: go straight to calculated / indexed location.  
+ **Hashing**: record key → hash value → storage location.  
+ **Collision**: two keys produce same location.  
+ Collision methods: **linear probing**, **overflow area**, **chaining**.

## Floating-point
+ Number = **mantissa × 2^exponent**.  
+ Both mantissa and exponent use **two's complement**.  
+ Positive normalised mantissa starts **01**.  
+ Negative normalised mantissa starts **10**.  
+ More mantissa bits = **more precision**.  
+ More exponent bits = **greater range**.  
+ Fixed bits can cause **rounding error / truncation**.  
+ Too large = **overflow**.  
+ Too close to zero = **underflow**.
