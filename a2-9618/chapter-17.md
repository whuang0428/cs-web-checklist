...
Teach Chapter 17 in this order:

1. plaintext / ciphertext / encryption vocabulary  
2. symmetric cryptography  
3. asymmetric cryptography  
4. two key-use scenarios: private message vs verified message  
5. digital certificate and CA  
6. SSL/TLS as a real application  
7. digital signatures  
8. quantum cryptography benefits/drawbacks  

Do not start with TLS details. Students understand TLS better after public/private key and certificate concepts are clear.

---

## 17.2 Most important classroom warning

Students usually reverse key direction.

Use this memory rule:

> **Privacy:** encrypt with receiver's public key.  
> **Proof:** sign with sender's private key.

| Goal | Key used first |
| --- | --- |
| Keep message secret for receiver | receiver's public key |
| Prove sender identity | sender's private key |
...
+ how public/private keys are used
+ how a digital certificate is acquired
+ what a digital certificate contains
+ why SSL/TLS is used in online banking
+ difference between symmetric and asymmetric encryption
+ benefit/drawback of quantum cryptography
+ how a digital signature verifies message integrity

The main revision mistake is over-learning cryptographic mathematics but under-learning mark scheme wording.

---

## 17.4 Minimal full-mark phrase set

Students should memorise these phrase patterns:

1. **Encryption:** plaintext is converted to ciphertext using an algorithm and key.  
2. **Symmetric:** the same shared key is used to encrypt and decrypt data.  
3. **Asymmetric:** a public/private key pair is used; one key encrypts and the matching key decrypts.  
4. **Private message:** encrypt with receiver's public key; decrypt with receiver's private key.  
5. **Digital signature:** sign/hash using sender's private key; verify using sender's public key.  
6. **Digital certificate:** issued by a CA and links an identity to a public key.  
7. **TLS:** provides secure encrypted client-server communication and authenticates the server.  
8. **Quantum:** eavesdropping can be detected because observation changes quantum states.  
...
| Symmetric cryptography | same shared key used for encryption and decryption |
| Asymmetric cryptography | public/private key pair used; one key encrypts and matching key decrypts |
| Digital certificate | electronic document issued by a CA linking identity to public key |
| Digital signature | value created using sender private key to verify sender and message integrity |
| SSL/TLS | protocol used to provide secure client-server communication |
| Quantum cryptography | uses quantum properties to provide secure communication / detect eavesdropping |

## 18.2 Key direction table

| Purpose | Encrypt / sign with | Decrypt / verify with |
| --- | --- | --- |
| Send private message to receiver | receiver's public key | receiver's private key |
| Prove sender identity | sender's private key | sender's public key |
| TLS data transfer after handshake | symmetric session key | same symmetric session key |

## 18.3 Symmetric vs asymmetric

| Symmetric | Asymmetric |
| --- | --- |
| one shared key | public/private key pair |
| fast | slower |
| good for large data | good for key exchange/signatures |
| key distribution problem | public key can be shared |

## 18.4 Digital certificate checklist
...
+ **browser/client verifies certificate**
...
## 18.6 Quantum cryptography checklist

| Benefit | Drawback |
| --- | --- |
| eavesdropping can be detected | expensive equipment |
| based on quantum physics | limited distance |
| very secure key exchange | still developing / not widely deployed |
...
+ explain symmetric cryptography in one sentence
+ explain asymmetric cryptography in one sentence
+ describe how to send a private message using public/private keys
+ describe how to send a verified message using digital signature
+ explain the role of Certificate Authority
+ list what a digital certificate contains
+ explain why HTTPS / online banking uses SSL/TLS
+ compare symmetric and asymmetric cryptography
+ give one benefit and one drawback of quantum cryptography
+ avoid mixing up public key and private key directions
